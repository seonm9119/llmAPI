OVERALL_PROMPT_VERSION = "resume-overall-v11"
...
- evaluationPlan.summaryFrame contains the planned title, overallJudgement, strongestArea, documentAiJudgement, medicalVisionJudgement, serviceJudgement, problemFramingJudgement, verificationLimits, levelJudgement, bestFitRoles, and oneLineEvaluation.
...
- Synthesize the candidate-level pattern across multiple projects: Document AI/OCR, medical or computer vision modelling, evaluation practice, and API/frontend productization.
...
- The middle sentences should cover these sections in compressed prose: overall judgement, strongest area, Document AI/OCR evidence, medical/computer-vision evaluation, serviceization pattern, weak points, level judgement, and best-fit roles.
...
- Translate those details into plain capability language such as document classification verification, controlled report generation, medical-image evaluation, service API, and review workflow.
...
- Prefer this style of judgement: "여러 프로젝트를 종합하면, 모델 실험을 결과표로 끝내지 않고 검증 기준, 비교 화면, API, 프론트엔드 검토 흐름까지 연결하는 엔지니어링 성향이 뚜렷합니다."
